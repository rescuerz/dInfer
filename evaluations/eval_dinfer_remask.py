...
from dinfer import ThresholdParallelDecoder,CreditThresholdParallelDecoder, HierarchyDecoder, SlideWindowRCRDecoder, BlockWiseDiffusionLLM, IterSmoothDiffusionLLM, VicinityCacheDiffusionLLM, IterSmoothWithVicinityCacheDiffusionLLM, BlockDiffusionLLM
...
class EvalConfig:
    ...
    # SlideWindowRCRDecoder parameters
...
class DInferEvalHarness(LM):
    def __init__(
        ...
        **kwargs
        ...
        elif parallel_decoding == "slide_window_rcr":
        ...
        # Store decoder reference for statistics access
    ...
    def generate_until(self, requests):
        ...
        def run_benchmark(world_size, rank, gpu_id, tokenizer, args):
            ...
            with set_current_vllm_config(VllmConfig(parallel_config = parallel_config)):
                ...
                elif args.parallel_decoding == 'slide_window_rcr':
                    decoder = SlideWindowRCRDecoder(
                        temperature=0,
                ...
                for wi in range(1):
                    ...
                    for i in iterator:   
                        ...
                        # 记录 SlideWindowRCRDecoder 统计信息（用于计算当前 sample 的增量）
                        prev_remask_stats = {}
                        if args.parallel_decoding == 'slide_window_rcr' and hasattr(decoder, 'get_stats'):
                            prev_stats = decoder.get_stats()
                            prev_remask_stats = {k: v for k, v in prev_stats.items()}
                        out = dllm.generate(input_ids, gen_length=min_padded_length, block_length=block_length)
                        nfe = dllm.num_forwards - prev_forwards
                        # 计算当前 sample 的 SlideWindowRCRDecoder 统计增量
                        sample_remask_stats = {}
                        if args.parallel_decoding == 'slide_window_rcr' and hasattr(decoder, 'get_stats'):
                            curr_stats = decoder.get_stats()
                            sample_remask_stats = {k: curr_stats[k] - prev_remask_stats.get(k, 0) for k in curr_stats}
                        ...
                        if rank == 0:
                            ...
                            # 打印当前 sample 的 SlideWindowRCRDecoder remask 统计增量和累积值
                            if args.parallel_decoding == 'slide_window_rcr' and sample_remask_stats:
                                cumulative_stats = decoder.get_stats()
                                print(f'         [Remask] sample: low_conf={sample_remask_stats["remask_low_conf_count"]}, declining={sample_remask_stats["remask_declining_count"]}, consec_dec={sample_remask_stats["remask_consecutive_declining_count"]}, total={sample_remask_stats["total_remask_count"]} | cumulative: low_conf={cumulative_stats["remask_low_conf_count"]}, declining={cumulative_stats["remask_declining_count"]}, consec_dec={cumulative_stats["remask_consecutive_declining_count"]}, total={cumulative_stats["total_remask_count"]}')
                            if wi==0 and i<5:
                                for j in range(input_ids.shape[0]):
                ...
                if rank==0:
                    ...
                    # Print SlideWindowRCRDecoder statistics if applicable
                    if args.parallel_decoding == 'slide_window_rcr' and hasattr(decoder, 'get_stats'):
                        stats = decoder.get_stats()
                        print("=" * 60)
                        print("[SlideWindowRCRDecoder Remask Statistics]")
                        ...
                        print(f"  Consecutive declining remask: {stats['remask_consecutive_declining_count']}")
                    ...
                    with open (filename, 'w') as f:
                        for i in range(len(answers)):
                            answer = answers[i]
                            save_data = {'answer': answer, 'generated_length': token_numbers[i], 'tpf':tpfs[i//args.batch_size], 'tps':tpss[i//args.batch_size], 'fps':fpss[i//args.batch_size]}
                            # Add SlideWindowRCRDecoder stats to saved data (only for first answer to avoid duplication)
                            if i == 0 and args.parallel_decoding == 'slide_window_rcr' and hasattr(decoder, 'get_stats'):
                                stats = decoder.get_stats()
                                save_data['remask_low_conf_count'] = stats['remask_low_conf_count']
                                save_data['remask_declining_count'] = stats['remask_declining_count']
                                save_data['remask_consecutive_declining_count'] = stats['remask_consecutive_declining_count']
                                save_data['total_remask_count'] = stats['total_remask_count']
                    ...
                    if self.show_speed and self.save_dir is not None:
                        ...
                        # Add SlideWindowRCRDecoder stats to saved data
                        if args.parallel_decoding == 'slide_window_rcr' and hasattr(decoder, 'get_stats'):
                            stats = decoder.get_stats()
                            data['remask_low_conf_count'] = stats['remask_low_conf_count']
                            data['remask_declining_count'] = stats['remask_declining_count']
                            data['remask_consecutive_declining_count'] = stats['remask_consecutive_declining_count']
                            data['total_remask_count'] = stats['total_remask_count']
        ...
        if self.parallel == 'dp':
            with set_current_vllm_config(self.vllm_config):
                ...
                for i, req in enumerate(tqdm(requests, desc="Generating...")):
                    ...
                    # 记录 SlideWindowRCRDecoder 统计信息（用于计算当前 sample 的增量）
                    prev_remask_stats = {}
                    if self.parallel_decoding == 'slide_window_rcr' and hasattr(self.decoder, 'get_stats'):
                        prev_stats = self.decoder.get_stats()
                        prev_remask_stats = {k: v for k, v in prev_stats.items()}
                    ...
                    sample_remask_stats = {}
                    if self.parallel_decoding == 'slide_window_rcr' and hasattr(self.decoder, 'get_stats'):
                        curr_stats = self.decoder.get_stats()
                        sample_remask_stats = {k: curr_stats[k] - prev_remask_stats.get(k, 0) for k in curr_stats}
                    inner_stop = time.time()
                    sample_time = inner_stop - inner_start
                    outputs.append(out)
                    ...
                    if self.rank == 0:
                        ...
                        # 打印当前 sample 的 SlideWindowRCRDecoder remask 统计增量和累积值
                        if self.parallel_decoding == 'slide_window_rcr' and sample_remask_stats:
                            cumulative_stats = self.decoder.get_stats()
                            print(f'         [Remask] sample: low_conf={sample_remask_stats["remask_low_conf_count"]}, declining={sample_remask_stats["remask_declining_count"]}, consec_dec={sample_remask_stats["remask_consecutive_declining_count"]}, total={sample_remask_stats["total_remask_count"]} | cumulative: low_conf={cumulative_stats["remask_low_conf_count"]}, declining={cumulative_stats["remask_declining_count"]}, consec_dec={cumulative_stats["remask_consecutive_declining_count"]}, total={cumulative_stats["total_remask_count"]}')
                    tpfs.append(tpf)
                    tpss.append(tps)
                    fpss.append(fps)
            ...
            if self.parallel_decoding == 'slide_window_rcr' and hasattr(self.decoder, 'get_stats'):
                ...
                print("=" * 60)
                print("[SlideWindowRCRDecoder Remask Statistics]")
                print(f"  Low confidence remask (conf < {self.low_threshold}): {stats['remask_low_conf_count']}")
                print(f"  Declining threshold remask (decline > {self.decline_threshold}): {stats['remask_declining_count']}")
                print(f"  Consecutive declining remask: {stats['remask_consecutive_declining_count']}")
                print(f"  Total remask count: {stats['total_remask_count']}")
                print("=" * 60)
