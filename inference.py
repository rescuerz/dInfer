...
# 启用历史专家路由复用功能
# 参数: (enabled, gamma, decay)
# gamma: 历史先验强度基础值
# decay: gamma 随迭代次数的衰减系数
model.set_history_routing(enabled=True, gamma=0.5, decay=0.9)
print("历史专家路由已启用: gamma=0.5, decay=0.9")
...
prev_forwards = dllm.num_forwards  # 记录生成前的 forward 次数
res = dllm.generate(input_ids, gen_length=1024, block_length=64)
nfe = dllm.num_forwards - prev_forwards  # 计算本次生成的 NFE
end_time = time.time()
elapsed_time = end_time - start_time
print(f"生成耗时: {elapsed_time:.2f} 秒")
print(f"NFE (Number of Forward Evaluations): {nfe}")
print(f"FPS (Forwards Per Second): {nfe / elapsed_time:.2f}")
